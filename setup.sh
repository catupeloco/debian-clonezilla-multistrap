...
xfce4-battery-plugin xfce4-clipman      xfce4-clipman-plugin     xfce4-cpufreq-plugin     xfce4-cpugraph-plugin    xfce4-datetime-plugin    xfce4-diskperf-plugin \
...
thunar-archive-plugin      thunar-media-tags-plugin ntfs-3g  gdebi timeshift mate-polkit rofi \
${BUG_FIXES_PACKAGES} \
...
network-manager                        network-manager-openconnect network-manager-openconnect-gnome network-manager-l2tp openconnect           \
...
echo START - DISK $END_DISK_SETUP_START_DOWNLOADS - DOWNLOADS $END_DOWNLOADS_START_EXTRACTING_CLONEZILLA - CLONEZILLA $END_EXTRACTING_CLONEZILLA_START_MMDEBSTRAP \
 - MMDEBSTRAP $END_MMDEBSTRAP_START_POSTTASKS - POSTTASKS $END_POSTTASKS_START_CHROOT - CHROOT $END_CHROOT_START_SCRIPTS - END $END > $TIMES
...
#!/bin/bash
while true ; do
	echo Downloaded packages Information ---------------------
